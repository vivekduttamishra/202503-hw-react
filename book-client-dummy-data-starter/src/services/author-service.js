...
class AuthorService  {
    ...
    getById=(id)=>{
        return this._authors.findOne(b=>b.id.toLower()===id.toLower(),`Invalid Book Id:${id}`,{id})
    }
    ...
    getRandom=()=>{
        ...
        const randomIndex = Math.floor(Math.random() * this._authors.length);
        return this._authors[randomIndex];
    }

    search=(matcher)=>{
        return this._authors.filter(matcher);
    }

    remove=(id)=>{
        this._authors=this._authors.filter(b=>b.id.toLower()!==id);
    }

    update=(book)=>{
        this._authors=this._authors.map(b=> b.id===book.id ? book : b )
    }
