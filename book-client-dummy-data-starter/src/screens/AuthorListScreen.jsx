...
export default function AuthorListScreen() {
    return (
      <div className="container py-4">
        ...
        <div className="row">
          {authors.map((author) => (
            <div key={author.id} className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
              <div className="card">
                <Link to={`/authors/${author.id}`} className="text-decoration-none">
                <img src={author.photo} className="card-img-top" alt={author.name} />
                <div className="card-body">
                  <h5 className="card-title">{author.name}</h5>
                  <p className="card-text">{author.bio.length > 50 ? author.bio.substring(0, 50) + "..." : author.bio}</p>
                  <div>
                    {author.tags.map((tag, index) => (
                      <span key={index} className="badge bg-dark me-1">{tag}</span>
                    ))}
                  </div>
                </div>
                </Link>
